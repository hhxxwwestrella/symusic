...
from aria.embedding import get_global_embedding_from_midi, get_global_embeddings_from_midi_batch
from new_mididict import MidiDict_allowing_buffer
...
def encode_contexts(args):
    ...
    def _build_head_from_state_dict(head_sd: dict):
        w1 = head_sd['0.weight']; w2 = head_sd['2.weight']
        base_dim, hid, proj_dim = w1.shape[1], w1.shape[0], w2.shape[0]
        head = nn.Sequential(nn.Linear(base_dim, hid), nn.ReLU(), nn.Linear(hid, proj_dim)).to(args.device)
        head.load_state_dict(head_sd); head.eval()
        return head, proj_dim
    context_head, proj_dim = _build_head_from_state_dict(ckpt["context_head"])

    # Prepare outputs
    out_dir = Path(args.out_dir); out_dir.mkdir(parents=True, exist_ok=True)
    meta_path = out_dir / "contexts.tsv"
    vec_path  = out_dir / "contexts.f32"
    if vec_path.exists(): os.remove(vec_path)
    wf_meta = open(meta_path, "w", encoding="utf-8")

    batch_ids, batch_masks, batch_spans = [], [], []
    ...
    def flush_batch():
        ...
        results = []
        for i in range(0, len(batch_ids), micro_bs):
            sub_ids = batch_ids[i:i+micro_bs]

            lens = [len(x) for x in sub_ids]
            L = max(lens)
            B = len(sub_ids)

            ids_pad = np.zeros((B, L), dtype=np.int64)
            msk_pad = np.zeros((B, L), dtype=np.int64)
            for j, arr in enumerate(sub_ids):
                ids_pad[j, :len(arr)] = arr
                msk_pad[j, :len(arr)] = 1

            ids_t = torch.from_numpy(ids_pad).to(args.device, non_blocking=True)
            msk_t = torch.from_numpy(msk_pad).to(args.device, non_blocking=True)

            with torch.no_grad(), torch.amp.autocast("cuda", enabled=True):
                out = _aria_forward(aria, ids_t, msk_t)

            if out.dim() == 3:
                m = msk_t.unsqueeze(-1).float()
                base = (out * m).sum(1) / m.sum(1).clamp_min(1e-6)
            else:
                base = out

            z = context_head(base)
            z = torch.nn.functional.normalize(z, p=2, dim=1)
            results.append(z.detach().cpu().numpy().astype("float32"))

        z_all = np.concatenate(results, axis=0)

        with open(vec_path, "ab") as f:
            f.write(z_all.tobytes())

        for (meta, span) in batch_spans:
            wf_meta.write(f"{meta['src_path']}\t{span[0]}\t{span[1]}\n")

        batch_ids.clear()
        batch_spans.clear()

    # Iterate shards with per-shard progress bar
    shard_paths = sorted(glob.glob(str(args.shards_dir / "tokens-*.tar*")))
    for sp in shard_paths:
        with tarfile.open(sp, "r:*") as tf:
            metas = [m for m in tf.getmembers() if m.name.endswith(".meta.json")]
            with tqdm(total=len(metas), desc=f"Shard {Path(sp).name}", ncols=100) as pbar:
                for m in metas:
                    ...
                    try:
                        ...
                        cid = np.load(io.BytesIO(get_bytes(f"{stem}.context_ids.npy")), allow_pickle=False)
                        ...
                        batch_spans.append((meta, span))
                        if len(batch_ids) >= 512:
                            flush_batch()
                    except Exception as e:
                        print(f"Failed item in {sp}: {e}")
                    finally:
                        pbar.update(1)
        flush_batch()

    wf_meta.close()
    print(f"Done. Vectors -> {vec_path}, meta -> {meta_path}")
    print(f"To load vectors: np.fromfile('{vec_path}', dtype=np.float32).reshape(-1, {proj_dim})")

### IF work on extracted shard dir
# @torch.no_grad()
# def encode_contexts(args):
#     # Load Aria backbone
#     model_cfg = ModelConfig(**load_model_config(name="medium-emb"))
...
#
#     def _build_head_from_state_dict(head_sd: dict):
#         w1, w2 = head_sd["0.weight"], head_sd["2.weight"]
#         base_dim, hid, proj_dim = w1.shape[1], w1.shape[0], w2.shape[0]
#         head = nn.Sequential(
...
#         batch_spans.clear()
#
#     # Iterate extracted shard directories
#     shard_dirs = sorted(glob.glob(str(args.shards_dir / "tokens-*")))
#     for d in shard_dirs:
#         d = Path(d)
#         metas = sorted(d.glob("*.meta.json"))
#         with tqdm(total=len(metas), desc=f"Shard {d.name}", ncols=100) as pbar:
#             for m in metas:
#                 try:
#                     meta = json.load(open(m, "r", encoding="utf-8"))
#                     stem = str(m)[:-len(".meta.json")]
#                     cid = np.load(f"{stem}.context_ids.npy", allow_pickle=False)
#                     ids = cid.astype(np.int64, copy=False)
#
#                     if ids.size == 0:
#                         pbar.update(1)
...
def main():
    ...
    s = sub.add_parser("encode-index-shards")
    s.add_argument("--shards-dir", type=Path, required=True)
    s.add_argument("--aria-ckpt", type=str, required=True)
    s.add_argument("--proj-ckpt", type=Path, required=True)
    s.add_argument("--out-dir", type=Path, default=Path("indices/ctx_shard_new"))
    s.add_argument("--k-bars", type=int, default=12)
    ...
    elif args.cmd == "encode-index-shards": encode_contexts(args)
    else: query(args)
